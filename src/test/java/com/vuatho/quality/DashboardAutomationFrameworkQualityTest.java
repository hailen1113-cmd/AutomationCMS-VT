...
public class DashboardAutomationFrameworkQualityTest {
     ...
     */
    @Test(description = "DASH-001 - Dashboard locators do not depend on dynamic IDs")
     ...
     */
    @Test(description = "DASH-002 - Dashboard locators avoid full XPath and long Tailwind chains")
     ...
     */
    @Test(description = "DASH-003 - Dashboard flow does not use Thread.sleep")
    public void dashboardFlowUsesExplicitWaits() throws IOException {
        String source = source("pages", "DashboardPage.java")
                + source("components", "SidebarComponent.java")
                + source("support/dashboard", "DashboardTestSupport.java")
                + source("tests/dashboard", "DashboardCoreUiTest.java")
                + source("tests/dashboard", "DashboardSummaryCardsTest.java")
                + source("tests/dashboard", "DashboardWorkbookUiTest.java")
                + source("tests/dashboard", "DashboardLogoutTest.java");
     ...
     */
    @Test(description = "DASH-004 - Independent tests continue after one failure")
     ...
     */
    @Test(description = "DASH-005 - Browser lifecycle is controlled at suite level")
    public void browserLifecycleIsCentralized() throws IOException {
        ...
        Assert.assertTrue(lifecycle.contains("finishExecution"));
        Assert.assertTrue(session.contains("releaseAfterSuite"));
     ...
     */
    @Test(description = "DASH-006 - Failure evidence includes screenshot reporting")
